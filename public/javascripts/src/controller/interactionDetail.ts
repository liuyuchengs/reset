...
function interactionDetail($scope:any,$rootScope:IRootScope.rootScope,$location:ag.ILocationService,ToolService:Tool,AjaxService:Ajax){
    $scope.post = {};
    $scope.replyMess = {};
    ...
    //切换预览显示
    $scope.switchSwiper = (item:any,has:any)=>{
    ...
    // 检查评论信息
    let mergeMessage = (post:any)=>{
        post.message = "";
        var comment  = post.commentList;
        if($scope.replyMess.commentList){
            var counts:any = $scope.replyMess.commentList.length;
        }else{
            var counts:any = 0;
        }
        if(comment.length>0){
    ...
    // 显示评论回复输入框
    $scope.showReplyInput = (id:any,replyId:any,replyName:any)=>{
        if(checkLogin()){
    ...
    // 发表回复
    $scope.sendReply = (content:any,id:any)=>{
        if(content==""||content==null){
            ToolService.alert("消息为空！");
        }else{
            var params:any = {
    ...
    // 跳转到医生页面
    $scope.goDoctor = (id:any)=>{
        ToolService.changeRoute("/doctor/detail","id="+id);
    }

    // 跳转到项目页面
    $scope.goProduct = (productId:any,hospitalId:any)=>{
