...
function productDetail($scope:any,$rootScope:IRootScope.rootScope,$location:ag.ILocationService,$http:ag.IHttpService,ToolService:Tool,AjaxService:Ajax){
    ...
    let queryDoctor = ()=>{
        AjaxService.post({
            ...
            if(data.list.length>0){
                ...
                $scope.doctorInfo.forEach((item:any)=>{
    ...
    // 查询医生排班信息
    let queryDoctorSchedule = (docId:any)=>{
    ...
    // 选择就医时间按钮
    $scope.chooseSchedule = (docId:any,name:any)=>{
    ...
    // 改变就医时间的选取值
    $scope.mergeDocSelect = (docId:any,value:any)=>{
        $scope.doctorInfo.forEach((item:any)=>{
            if(item.id==docId){
