...
function doctorDetail($scope:any,$rootScope:IRootScope.rootScope,$location:ag.ILocationService,$anchorScroll:ag.IAnchorScrollService,AjaxService:Ajax,ToolService:Tool){
    $scope.hasTime = false;
    ...
    // swiper跳转到指定页
    $scope.toPage = (params:any,$event:any)=>{
        if(params==="per"){
    ...
    // 检查医生排班信息
	let mergeSchedule = (items:any[])=>{
        var initBoolean = true;
        items.forEach(function (item) {
            let childrenList:any[] = [];
            if(item.timeList.length>0){
                item.timeList.forEach((childrenItem:any)=>{
                    childrenList.push({
                        has:false,
    ...
    // 切换导航栏
    $scope.switchDay = (index:any,$event:ag.IAngularEvent)=>{
            ToolService.select(index, $scope.time.timeList);
            $event.stopPropagation();
        };

    // 选择时间
    $scope.selectTime = (index:any,obj:any,$event:ag.IAngularEvent)=>{
        $event.stopPropagation();
    ...
    // 获取money以及跳转页面
    $scope.orderDetail = (id:any,title:any,price:any)=>{
